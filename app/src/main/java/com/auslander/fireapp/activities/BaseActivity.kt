...
import com.auslander.fireapp.Base
import com.auslander.fireapp.extensions.observeChildEvent
import com.auslander.fireapp.extensions.observeSingleValueEvent
import com.auslander.fireapp.extensions.setValueRx
import com.auslander.fireapp.extensions.toMap
import com.auslander.fireapp.model.constants.DBConstants
import com.auslander.fireapp.utils.*
import com.auslander.fireapp.utils.enc.DecryptionHelper
import com.auslander.fireapp.utils.enc.EncryptionHelper
import com.auslander.fireapp.utils.enc.MessageDecryptor
import com.auslander.fireapp.utils.network.FireManager
import com.auslander.fireapp.utils.update.UpdateChecker
...
abstract class BaseActivity : AppCompatActivity(), Base {
    ...
    private lateinit var sentMessageHandler: SentMessageHandler
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (!needsUpdate) {
            ...
            sentMessageHandler =
                SentMessageHandler(this, fireManager, messageDecryptor, disposables)
            ...
            if (SharedPreferencesManager.isDeletedUnfetchedMessage()) {
                ...
                if (SharedPreferencesManager.isDesktopEnabled()) {
                    attachDesktopMessagesListener()
                } else {
                    if (FireManager.isLoggedIn()) {
                        FireConstants.desktopUsers.child(FireManager.uid).observeSingleValueEvent()
                            .subscribe({ snap ->
                                SharedPreferencesManager.setDesktopEnabled(snap.exists())
                                if(snap.exists()){
                                   attachDesktopMessagesListener()
                                }
                            }, { error -> })
    ...
    private fun attachDesktopMessagesListener() {
        FireConstants.unSyncedDesktopMessages.child(FireManager.uid).observeChildEvent()
            .subscribe({ snap ->
                ...
                if (dataSnapshot.value != null) {
                    (dataSnapshot.child(DBConstants.MESSAGE_ID).value as? String)?.let { messageId ->
                        ...
                        val message = MessageMapper.mapToSentMessage(dataSnapshot)
                        lifecycleScope.launch(Main) {
                            sentMessageHandler.handleNewMessage(phone, message)
                        }

                        deleteSentMessage(messageId).subscribe().addTo(disposables)
                    }
    ...
    private fun deleteSentMessage(messageId: String): Completable {
        return FireConstants.unSyncedDesktopMessages.child(FireManager.uid).child(messageId)
            .setValueRx(null)
    }
