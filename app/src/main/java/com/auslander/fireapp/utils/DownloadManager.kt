...
import com.auslander.fireapp.job.NetworkJobService
...
import com.auslander.fireapp.utils.enc.MessageEncryptor
import com.auslander.fireapp.utils.mediastore.MessageMediaStore
import com.auslander.fireapp.utils.network.FireManager.Companion.getRef
...
class DownloadManager {
    ...
    suspend fun request(message: Message, scope: CoroutineScope, onComplete: OnComplete?) {
        ...
        if (MessageType.isSentType(type)) {
            if (message.src != "local") {
                downloadSentMessage(message, onComplete)
            } else {
                when (type) {
    ...
    private fun downloadSentMessage(message: Message, onComplete: OnComplete?) {
        val type = message.type
        val link = message.content
        val messageId = message.messageId
        val receiverId = message.chatId
        ...
        val file: File =
            when (type) {
                MessageType.SENT_FILE -> DirManager.generateFileForFilesType(
                    type,
                    Util.getFileNameFromPath(link)
                )

                MessageType.SENT_AUDIO -> DirManager.generateAudioFile(
                    type,
                    Util.getFileExtensionFromPath(link)
                )
