...
import com.auslander.fireapp.extensions.updateChildrenRx
import com.auslander.fireapp.model.ExpandableContact
import com.auslander.fireapp.model.PhoneContact
...
//    }
    ...
    @JvmStatic
    fun syncContacts(syncDesktopUsers: Boolean): Completable {
        val realmHelper = RealmHelper.getInstance()
        return Completable.create { emitter ->

            val contactsMap = hashMapOf<String, Any>()
            fetchContacts().subscribeOn(Schedulers.io()).subscribe({ user ->

                if (syncDesktopUsers) {
                    contactsMap[user.uid] = mapOf(
                        "name" to user.userName,
                        "number" to user.phone
                    )
                }
                ...
                if (contactsMap.isNotEmpty()) {
                    FireConstants.contacts.child(FireManager.uid).setValueRx(contactsMap)
                        .subscribe({
    ...
    fun deleteDeletedUsers(): Observable<User> {
        val realmHelper = RealmHelper.getInstance()
        return Observable.fromIterable(realmHelper.listOfUsers).flatMap { user ->
            val uid = user.uid
            return@flatMap FireConstants.deletedUsers.child(uid).observeSingleValueEvent()
                .toObservable()
                .map { Pair(it, uid) }
        }.flatMap { (snap, uid) ->
            if (snap.exists()) {
                realmHelper.setUserDeleted(uid, true)
            }
