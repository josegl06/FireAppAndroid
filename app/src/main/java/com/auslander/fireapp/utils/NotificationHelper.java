...
import static com.auslander.fireapp.utils.IntentUtils.NOTIFICATION_ACTION_DECLINE;
import static com.auslander.fireapp.utils.MessageTypeHelper.getMessageContent;
...
import com.auslander.fireapp.activities.BackupChatActivity;
import com.auslander.fireapp.activities.calling.CallingActivity;
import com.auslander.fireapp.activities.main.MainActivity;
import com.auslander.fireapp.activities.main.messaging.ChatActivity;
...
import com.auslander.fireapp.receivers.HandleReplyReceiver;
import com.auslander.fireapp.receivers.MarkAsReadReceiver;
import com.auslander.fireapp.services.CallingService;
...
public class NotificationHelper extends ContextWrapper {
    ...
    private PendingIntent getPendingIntent(Chat chat) {
        ...
        int flagUpdateCurrent = BuildVerUtil.isApi31OrAbove() ? PendingIntent.FLAG_UPDATE_CURRENT | PendingIntent.FLAG_MUTABLE : PendingIntent.FLAG_UPDATE_CURRENT;
        if (isBelowApi24()) {
            ...
            if (chat != null) {
                ...
                pendingIntent = stackBuilder.getPendingIntent(ID_NOTIFICATION, flagUpdateCurrent);
            } else {
                //otherwise there are multiple messages from multiple users therefore just open Main Activity
                Intent intent = new Intent(this, MainActivity.class);
                pendingIntent = PendingIntent.getActivity(this, 1, intent, flagUpdateCurrent);
            }
...
//        start chatActivity with the clicked user
            ...
            TaskStackBuilder stackBuilder = TaskStackBuilder.create(this);
            stackBuilder.addNextIntentWithParentStack(intent);
            pendingIntent = stackBuilder.getPendingIntent(chat.getNotificationId(), flagUpdateCurrent);
        }
    ...
    public NotificationCompat.Builder createRestoreNotification(int notificationId, int progress) {
        ...
        int flagUpdateCurrent = BuildVerUtil.isApi31OrAbove() ? PendingIntent.FLAG_UPDATE_CURRENT | PendingIntent.FLAG_MUTABLE : PendingIntent.FLAG_UPDATE_CURRENT;
        PendingIntent pi = PendingIntent.getActivity(this, notificationId, backupChatIntent, flagUpdateCurrent);
        NotificationCompat.Builder mBuilder =
    ...
    public Notification createActiveCallNotification(FireCall fireCall, int notificationId) {
        ...
        int flagUpdateCurrent = BuildVerUtil.isApi31OrAbove() ? PendingIntent.FLAG_UPDATE_CURRENT | PendingIntent.FLAG_MUTABLE : PendingIntent.FLAG_UPDATE_CURRENT;
        PendingIntent hangupPIntent = PendingIntent.getActivity(this, notificationId, hangupIntent, flagUpdateCurrent);
    ...
    private PendingIntent getNotificationClickPendingIntent(FireCall fireCall, int action, int requestCode) {
        ...
        int flagUpdateCurrent = BuildVerUtil.isApi31OrAbove() ? PendingIntent.FLAG_UPDATE_CURRENT | PendingIntent.FLAG_MUTABLE : PendingIntent.FLAG_UPDATE_CURRENT;
        return PendingIntent.getActivity(this, requestCode, notificationIntent, flagUpdateCurrent);
    }
...
//        PendingIntent pendingIntent = PendingIntent.getService(this, PI_REQUEST_CODE_ANSWER, answerIntent, PendingIntent.FLAG_UPDATE_CURRENT);
        ...
        int flagUpdateCurrent = BuildVerUtil.isApi31OrAbove() ? PendingIntent.FLAG_UPDATE_CURRENT | PendingIntent.FLAG_MUTABLE : PendingIntent.FLAG_UPDATE_CURRENT;
        PendingIntent declinePIntent = PendingIntent.getService(this, PI_REQUEST_CODE_DECLINE, declineIntent, flagUpdateCurrent);
