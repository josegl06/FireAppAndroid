...
import com.auslander.fireapp.common.extensions.tint
    ...
    open fun bind(message: Message, user: User) {
        ...
        if (quotedMessageFrame != null) {
            if (message.quotedMessage == null) {
                ...
                quotedMessageFrame.setBackgroundColor(
                    ContextCompat.getColor(
                        context,
                        R.color.quoted_received_background_color
                    )
                )
                tvQuotedName?.setTextColor(
                    ContextCompat.getColor(
                        context,
                        R.color.quoted_received_text_color
                    )
                )
                tvQuotedText?.setTextColor(ContextCompat.getColor(context, R.color.colorText))
                quotedColor?.setBackgroundColor(
                    ContextCompat.getColor(
                        context,
                        R.color.quoted_received_quoted_color
                    )
                )
        ...
        val progressIdleIconRes =
            if (MessageType.isSentType(message.type) && message.src != null && message.src == "local") R.drawable.ic_file_upload else R.drawable.ic_file_download
...
private fun hideOrShowProgressView(
    progressButton: DownloadButtonProgress?,
    tvSize: TextView?,
    stat: Int
) {
